  ...
  useEffect(() => {
    ...
    httpBridge.start(5561, 'http_service', request => {
      ...
      // you can use request.url, request.type and request.postData here
        ...
        request.type === 'GET' &&
        requestUrlComponents[1] === 'session'
      ) {
        if (requestUrlComponents.length >= 3) {
          ...
          `);
              ...
              const userActionsSteps =
                DataParser.getUserActionData(sessionData);
              console.log('userActionsSteps: ', userActionsSteps);

              let responseHtml = html3.replace(
                '{{user_actions}}',
                userActionsSteps,
              );
          ...
          `);
            ...
            httpBridge.respond(
              ...
              200,
              'text/html',
              html2.replace('{{session_data}}', sessionData.reverse().join('')),
            );
          });
        }
      } else {
